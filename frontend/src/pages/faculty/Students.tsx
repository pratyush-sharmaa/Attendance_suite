...
interface Student {
  ...
  parent_email: string; parent_name: string; parent_phone: string
}
...
export default function FacultyStudents() {
  ...
  const openEdit = (s: Student) => {
    ...
    setForm({
      ...
      parent_email: s.parent_email || '', parent_name: s.parent_name || '', parent_phone: s.parent_phone || ''
    })
    setPhoto(null)
    setPreview(null)
    setShowForm(true)
  ...
  const handleSave = async () => {
    if (!form.name || !form.roll_no) { showToast('Name and roll number are required', 'error'); return }
    if (!editStudent && !photo)      { showToast('Please upload a face photo', 'error'); return }
      ...
      <div style={{ marginLeft: 240, flex: 1, padding: '32px 36px' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
          <div>
            ...
            <p style={{ color: '#475569', marginTop: 4, fontSize: '0.875rem' }}>Register and manage students</p>
          </div>
          <button
        ...
        {sections.length === 0 && (
          <div style={{
            ...
            borderRadius: 12, padding: '16px 20px', color: '#fbbf24', marginBottom: 24, fontSize: '0.875rem'
          }}>
          ...
          <div style={{ display: 'flex', gap: 8, marginBottom: 24, flexWrap: 'wrap' }}>
            {sections.map(sec => (
              <button key={sec.id} onClick={() => { loadStudents(sec); closeForm() }} style={{
                ...
                🏫 {sec.name} <span style={{ opacity: 0.7, marginLeft: 4 }}>({sec.student_count})</span>
              </button>
            ))}
          </div>
        )}
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24, marginBottom: 20 }}>
              ...
              <div>
                ...
                <label style={{
                  ...
                  {preview
                    ? <img src={preview} alt="preview" style={{ width: 120, height: 120, borderRadius: '50%', objectFit: 'cover', border: '3px solid #6366f1' }} />
                    : <>
                        <div style={{ fontSize: 36, marginBottom: 8 }}>📸</div>
                        <div style={{ color: '#475569', fontSize: '0.85rem' }}>{editStudent ? 'Click to upload new photo' : 'Click to upload face photo'}</div>
        ...
        <div className="card">
          ...
          {loading ? (
            ...
            <div style={{ textAlign: 'center', padding: '48px', color: '#475569' }}>
              ...
              <p style={{ marginBottom: 16 }}>{search ? 'No match' : 'No students yet'}</p>
              {!search && <button className="btn-primary" onClick={openAdd}>+ Register First Student</button>}
            </div>
          ) : (
            <table>
              <thead>
                <tr><th>#</th><th>Student</th><th>Roll No</th><th>Parent Email</th><th>Phone</th><th>Registered</th><th>Actions</th></tr>
              </thead>
              <tbody>
                {filtered.map((s, i) => (
                  <tr key={s.id}>
